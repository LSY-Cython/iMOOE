...
Run `exp_moe_train.py` and `exp_moe_test.py` to reproduce the iMOOE model on each PDE dataset. Every experiment can be independently manipulated by both `.yaml` configuration files in `configs/` folder and training/testing arguments in these two `.py` files.
